export default function FormatCaseNumber(caseData) {
  if(caseData){
    const {
      "Case No": caseNo,
...
const numericPart = caseNo.split('/')[0].match(/\d+/)?.[0] || ""; // Extracts the number before a slash
  ...
  const categoryFormats = {
    ...
    "CR-004-Hurt Cases": (number) => `SC - ${number}/II`,
    ...
    "CR-014-Offences Against Property": (number) => `SC - ${number}/II`,
    "CR-015-Sexual Offences": (number) => `SC - ${number}/II`,
  ...
  const formatFunction = categoryFormats[category] || categoryFormats["Other"];
