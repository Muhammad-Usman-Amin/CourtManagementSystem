...
//       backgroundColor: theme.palette.type === 'light' ? "lightgray" : theme.palette.background.paper,
//     },
//     tableEmptyCell: {
//       margin: 0,
//       padding: 0,
...
const PrintDisposal = (props) => {
  ...
  const handleExportToExcel = () => {
    const exportData = disposalCases.map((caseFile, i) => ({
      "S.No": serialNo[i],
      "Case No": caseFile["Case No"],
      "Case Title": caseFile["Case Title"],
      "Category Per PQS": caseFile["Category Per PQS"],
      "Date of Institution": caseFile["Date of Institution "]
        ? format(parseISO(caseFile["Date of Institution "]), "dd-MMM-yyyy")
        : "null",
      "Date of Transfer In":
        caseFile["Date of Transfer In"] &&
        parseISO(caseFile["Date of Transfer In"]).getFullYear() > 1980
          ? format(parseISO(caseFile["Date of Transfer In"]), "dd-MMM-yyyy")
          : "",
      "Date of Other Institution":
        caseFile["Date of Other Institution"] &&
        parseISO(caseFile["Date of Other Institution"]).getFullYear() > 1980
          ? format(
              parseISO(caseFile["Date of Other Institution"]),
              "dd-MMM-yyyy"
            )
          : "",
      "Institution Flag": caseFile["Institution Flag"] || "",
      "Date of Disposal Transfer Out":
        caseFile["Date of Disposal Transfer Out"] &&
        parseISO(caseFile["Date of Disposal Transfer Out"]).getFullYear() > 1980
          ? format(
              parseISO(caseFile["Date of Disposal Transfer Out"]),
              "dd-MMM-yyyy"
            )
          : "",
      "Disposal OR Transfer Out Flag":
        caseFile["Disposal OR Transfer Out Flag"] || "",
      "Disposal Mode Flag": caseFile["Disposal Mode Flag"] || "Not specified",
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Disposal Cases");

    XLSX.writeFile(
      workbook,
      `Disposal_Cases_${format(dateDisposal, "MMMM_yyyy")}.xlsx`
    );
  };
                  ...
                  disposalCases.map((caseFile, i) =>
                    ...
                    getSecondToLastElementCategory(caseFile.causeListEntries)
                      ...
                      <TableRow hover key={caseFile._id}>
                        ...
                        <TableCell className={classes.tableCell} align="left">
                          {caseFile["Disposal Mode Flag"] || "Not specified"}
                        </TableCell>
                      </TableRow>
                    ) : null
                  )}
